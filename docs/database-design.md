...
다음 항목은 구현 완료된 테이블이 아니라 V2 기능을 시작할 때 검증할 설계 후보이다. 화면 요구와 데이터 생명주기를 먼저 확정한 뒤 필요한 테이블만 추가한다.

### 자동 결재선

* `approval_template`: 결재 종류와 템플릿 기본 정보
* `approval_template_line`: 템플릿별 부서·직급·순서 규칙

템플릿은 상신 시 `approval_linetbl`을 생성하는 기준으로만 사용한다. 상신 이후 조직이나 템플릿이 변경되어도 기존 문서의 결재자가 바뀌지 않도록 실제 처리는 계속 문서별 결재선을 기준으로 한다.

### 알림과 협업

SSE 연결 자체는 영속 테이블이 필요하지 않지만 읽지 않은 알림, 재접속 후 복구와 전달 이력이 요구되면 알림 테이블을 검토한다. 결재건별 채팅이 실제로 필요해질 경우에만 채팅방과 메시지 테이블을 추가하고, 참여 권한과 보존 기간을 먼저 결정한다.

### 통계와 감사

초기 통계는 기존 결재·결재선·이력 테이블을 조회해 계산한다. 조회 비용이 실제 병목으로 확인되기 전에는 별도 집계 테이블을 만들지 않는다. 관리자 작업과 `SUPER_ADMIN` 전용 사용자별 통계 조회에는 조회자, 대상, 범위와 시각을 기록하는 감사 로그를 검토한다.

인사이동 이후에도 과거 통계를 상신 당시 조직 기준으로 유지하려면 문서 상신 시점의 부서 정보를 스냅샷으로 저장해야 한다. 현재 부서를 기준으로 다시 집계할지, 상신 당시 부서를 보존할지는 통계 API 구현 전에 확정한다.
